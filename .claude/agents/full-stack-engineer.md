...
| UI | `app/src/components/Timeline.tsx`, `components/editor/*` | Grid rendering + native HTML5 drag-and-drop (`Timeline.tsx`); `TileRow.tsx` (9-tile row + expand panel, anchored/dynamically-sized, real layout-flow growth) and `LogActivityModal.tsx` (duration drag-block, quality/symptoms/flag pickers, notes textarea) replace the picker/editor surface (`components/editor/`). A single monochrome light/dark theme (`styles/index.css`'s `bg`/`surface`/`ink`/`line`/`inv-*` tokens) applies app-wide — no per-category or per-item colour anywhere; the Sun/Moon icons beside the Day/Night timeline rows toggle it. Read/write only through `dispatch`. **`TileRow.tsx`/`LogActivityModal.tsx` still read the STATIC catalog** (`data/activities.ts`'s `CATEGORIES`/`ACTIVITY_CARDS`, `domain/types.ts`'s fixed `FlagId`/`ActivityQuality`/`Symptom` unions) — PICKER-CUSTOM-1 (below) built the real per-user backend and a separate management screen for it, but did NOT yet rewire the live picker/pickers to read from it; see that section for exactly what's pending. |
| Picker hierarchy (new, PICKER-CUSTOM-1) | `app/src/routes/ActivityLibraryPage.tsx`, `components/activityLibrary/*`, `domain/pickerHierarchy.ts`, `state/useTiles.ts`, `state/useActivityHierarchy.ts`, `state/useParameterOptions.ts`, `api/tiles.ts`, `api/activityHierarchy.ts`, `api/parameterOptions.ts` | The user-owned tile/activity/parameter-option CRUD surface — its own route (`Sidebar.tsx`'s "Activity Library" entry), not yet the source of truth for the live picker (see the UI row above and PICKER-CUSTOM-1 below). |
...
6. **Backend done, UI partial — PICKER-CUSTOM-1 (full user customization of the activity-picker hierarchy).** A new, independent layer on top of Phases 1–3 (not a replacement for any of them): any number of user-owned `tiles`, an arbitrary-depth user-owned `activities` tree (own `tile_id`/`sort_order`/`hidden`, unconditional per-user ownership for every NEW row — see the Target Architecture table above for why the legacy shared catalog rows are a deliberate, WORKFLOW.md-mandated expand-only exception, not an oversight), and per-activity `activity_parameter_options` (quality/symptom/flag) with inheritance, all live on the test Supabase project with full CRUD RPCs, history-safe delete-vs-hide, and `internal.assert_valid_quality`/`assert_valid_symptoms`/`assert_valid_flags` validating against the per-activity effective list instead of a hardcoded global one. A real management screen (`routes/ActivityLibraryPage.tsx`) exercises all of it. **Still pending**, deliberately scoped out of this pass (see `BACKLOG.md`): rewiring `TileRow.tsx`/`LogActivityModal.tsx`/`domain/types.ts`'s fixed `FlagId`/`ActivityQuality`/`Symptom` unions to actually READ from this new backend instead of the static `data/activities.ts` catalog — that swap touches the live picker's rendering, drag-and-drop, disappear rules and typed domain unions across most of the app's core files, and doing it in the same pass as building the backend was judged too much blast-radius for one review; do it as its own tightly-scoped follow-up once this has soaked.
