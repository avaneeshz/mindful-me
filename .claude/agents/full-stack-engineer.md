...
| UI | `app/src/components/Timeline.tsx`, `components/editor/*` | Grid rendering + native HTML5 drag-and-drop (`Timeline.tsx`); `TileRow.tsx` (reflowing tile grid + per-tile popup, any tile count) and `LogActivityModal.tsx` (duration drag-block, quality/symptoms/flag pickers, notes textarea) replace the picker/editor surface (`components/editor/`). A single monochrome light/dark theme (`styles/index.css`'s `bg`/`surface`/`ink`/`line`/`inv-*` tokens) applies app-wide — no per-category or per-item colour anywhere; the Sun/Moon icons beside the Day/Night timeline rows toggle it. Read/write only through `dispatch`. **`TileRow.tsx`/`LogActivityModal.tsx` now read the LIVE per-user catalog** once one is loaded (PICKER-CUSTOM-1's backend — see the Picker hierarchy row below), via `data/activities.ts`'s live-swappable registry (`setLiveActivityCatalog`/`findCard`/`cardsForCategory`/`effectiveCategories`/`effectiveCategoryOrder`), populated by `state/useLiveActivityCatalogSync.ts`. With zero backend configured (or before that first load resolves) every lookup falls back to the exact static `ACTIVITY_CARDS`/`CATEGORIES` catalog, byte for byte — rule 6 holds unchanged. |
| Picker hierarchy (PICKER-CUSTOM-1) | `app/src/routes/ActivityLibraryPage.tsx`, `components/activityLibrary/*`, `domain/pickerHierarchy.ts`, `state/useTiles.ts`, `state/useActivityHierarchy.ts`, `state/useParameterOptions.ts`, `state/useLiveActivityCatalogSync.ts`, `api/tiles.ts`, `api/activityHierarchy.ts`, `api/parameterOptions.ts` | The user-owned tile/activity/parameter-option CRUD surface (its own route, `Sidebar.tsx`'s "Activity Library" entry) AND, via `useLiveActivityCatalogSync`, the source of truth the LIVE day-to-day picker (`TileRow.tsx`/`LogActivityModal.tsx`) now reads from — the same `useTiles`/`useActivityHierarchy` hooks feed both surfaces, never two separate data paths. `domain/pickerHierarchy.ts`'s `buildActivityTree`/`activityNodeToCard`/`liveCategoriesFromTiles`/`liveActivityCardsFromRows` do the flat-rows-to-`ActivityCard`-tree conversion, arbitrary depth. |
...
**Database — the schedule itself lives in three tables; the picker HIERARCHY a user schedules against is a separate, now fully user-owned, layer (PICKER-CUSTOM-1, below):**

| Table | Holds | Key columns |
|---|---|---|
| `tiles` | A user's own top-level picker categories — any number, add/rename/hide/reorder/delete. Unconditionally owned (`created_by not null`) from day one, no shared-default phase. | `id, created_by, label, icon_key, sort_order, hidden` |
| `activities` | A user's own activity/sub-activity tree, arbitrary depth via `parent_id`, rooted at a `tiles` row. Still also holds the LEGACY shared catalog (`created_by is null`, `category_id` set) that real historical `scheduled_activities` rows may still reference — see PICKER-CUSTOM-1's expand-only note for why that hasn't been contracted away yet. | `id, name, tile_id, parent_id, icon_key, sort_order, hidden, created_by, category_id (deprecated)` |
| `activity_parameter_options` | Per-activity (or per-user fallback, `activity_id is null`) customizable option lists for quality/symptom/flag — replaces the old fixed 18/6/14-value global unions. Inheritance: own rows if any, else nearest ancestor's, else the fallback. | `id, created_by, activity_id, parameter_type, label, icon_key, sort_order` |
...
10. **Flags (the 14-value `FlagId` "Protective response" set — see `domain/types.ts`), activity quality, chronic symptoms, and freeform notes — every similarly sensitive field — are encrypted at rest**, sent only over HTTPS, and every query is scoped to the authenticated user — no cross-user reads, ever.
...
6. ✅ **Done — PICKER-CUSTOM-1 (full user customization of the activity-picker hierarchy), backend AND live-picker rewiring both.** A new, independent layer on top of Phases 1–3 (not a replacement for any of them): any number of user-owned `tiles`, an arbitrary-depth user-owned `activities` tree (own `tile_id`/`sort_order`/`hidden`/`disappear_mode`/`disappear_limit`, unconditional per-user ownership for every NEW row — see the Target Architecture table above for why the legacy shared catalog rows are a deliberate, WORKFLOW.md-mandated expand-only exception, not an oversight), and per-activity `activity_parameter_options` (quality/symptom/flag) with inheritance, all live on the test Supabase project with full CRUD RPCs, history-safe delete-vs-hide, and `internal.assert_valid_quality`/`assert_valid_symptoms`/`assert_valid_flags` validating against the per-activity effective list instead of a hardcoded global one. A real management screen (`routes/ActivityLibraryPage.tsx`) exercises all of it, AND the actual day-to-day picker (`TileRow.tsx`, `LogActivityModal.tsx`'s quality/symptom/flag pickers) now reads from this same backend live, via `data/activities.ts`'s live-swappable registry (see the UI/Picker-hierarchy rows above) — `domain/types.ts`'s `FlagId`/`ActivityQuality`/`Symptom` are now plain `string` (no longer closed unions; see that file's own doc comments for the full reasoning), `CategoryId` is plain `string` (was a fixed 9-value union), and `ActivityCard` gained a `children?: ActivityCard[]` field (`domain/boardReducer.ts`'s `isStagingComplete`/`stagingOptions` walk it to arbitrary depth — no more 3-level cap). `TileRow.tsx`'s tile row is a reflowing CSS grid (`grid-template-columns: repeat(auto-fill, minmax(72px, 1fr))`), not a fixed non-wrapping flex row, so it handles any tile count on any viewport. Zero-backend local-only mode (rule 6) is untouched — the live registry is only ever populated once Supabase is configured and a real fetch has resolved; every lookup falls back to the exact static catalog otherwise.

   **Known limitations, flagged rather than silently accepted:** (a) the live quality/symptom/flag picker in `LogActivityModal` scopes its effective option list to the staged activity's TOP-LEVEL card only, never a specific sub/third-level node — `api/catalog.ts`'s `catalogIdForName` (the only name-to-id resolver this codebase has) only ever resolves top-level names, so a sub-activity-specific parameter-option override (configurable in Activity Library) currently has no way to be looked up from the live modal; (b) no reorder-within-a-parameter-option-list or moving an activity to a different tile/parent after creation (both already noted in `BACKLOG.md`); (c) the Activity Library editor has no UI yet for setting a NEW activity's `disappear_mode`/`disappear_limit` (the DB/API support it — `create_activity`/`update_activity`'s trailing optional params — every user-created activity just defaults to `manual`, same as the server default).
