...
Your whole day, as two rows — Day (6a–6p) and Night (6p–6a) — with activities placed on a real timeline rather than a flat to-do list. Every activity carries an arbitrary duration (8 minutes, 37 minutes, 2 hours — never rounded to fit a grid), belongs to one of nine wellness categories (Sleep & Rest, Food & Nourishment, Personal Care, Downtime & Errands, Movement & Body Therapy, Work & Projects, Nature & Spirit, Growth & Connection, Home & Chores), and can optionally carry an activity quality, any number of chronic symptoms, a protective-response flag (Trauma response, Stress response, Fear response, Anger response), and freeform notes — the sensitive fields (flags, quality, symptoms, notes) encrypted at rest, visible only to you.

The whole interface is a single monochrome light/dark theme — no per-category or per-item colour anywhere — toggled instantly by the Sun/Moon icons beside the two timeline rows, and persisted per-device.
