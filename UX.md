...
**Activity quality, chronic symptoms, protective response, and notes — all optional, all live in the same modal.** Below the duration control sit three chip rows and a textarea, in this order: Activity quality, Chronic Symptoms (Pitta / Inflammation / Right knee pain / Calves pain / Temporal pain / Dryness — unlike the other two, any number at once, not single-select), and Protective response (Trauma Activation / Triggered / Attack / Anger / Procrastinated / Shut Down / Collapse / Over Accommodating / Hyper Responsibility / Over Function / Intellectualization / Optimization / Hyper Vigilance / Problem Solving, plus a real "None" chip ordered last — not an implicit empty state). All three chip rows render text-only, smaller labels — no icons — since the option lists are long. Below those, a plain always-visible "Add notes" textarea (no expand/collapse, no separate heading). Each field is set on the *individual activity being logged*, not a whole-slot toggle; changing any of them later is a normal edit like any other field. No section label sits above the duration ruler itself — it shows directly, unlabeled.
