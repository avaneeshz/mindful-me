...
**Steps migrated onto `public.daily_values` (synced).** Implemented, not yet committed or applied — see the full-stack-engineer session dated 2026-09-13. Closes the gap entry #2 below flagged ("Steps' history stays local-only ... that move is still a real decision ... not made here") and the one confirmed data-loss gap from a full app audit: Steps was the only user data that never reached the database, lost on every device switch/browser change/reinstall. `domain/displayButtons.ts`'s `steps` entry is now `synced: true`, mirroring `protein` exactly; `state/useDisplayValueHistory.ts`/`lib/displayValuesLocalStore.ts` no longer special-case Steps as the local-only one (both already branched on the generic `synced` flag, not the literal key). A one-time client-side backfill (`state/useStepsBackfill.ts`, wired into `HeaderBar`) pushes any pre-migration local-only Steps day up to the server on next load, once per browser that has such data — idempotent (checks the server's own list before pushing each date) and never overwrites a server value that's already there. Migration: `supabase/migrations/20260913080000_daily_values_steps.sql`, not applied — widens `daily_values.metric_key`'s CHECK constraint and `set_daily_value`'s validation from `('protein')` to `('protein', 'steps')`, same drop/add-constraint convention `20260910090000_note_entries_extra_buttons.sql` used for `note_entries.button_key`.
