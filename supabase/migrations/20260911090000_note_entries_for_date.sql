...
-- logged on ONE calendar day, across all 9 buttons at once. The only
-- existing read path, `list_note_entries(p_button_key)`, returns one
-- button's entire all-time history -- fetching all 9 buttons' full history
-- and filtering client-side to find one day's rows would mean loading a
-- user's full note-entry history to render one day, which is exactly what
-- rule 8 ("every read of 'today' or 'this week' is scoped to that window")
-- forbids. This adds a real date-scoped RPC instead, mirroring
-- `list_scheduled_activities(p_range_start, p_range_end)`
-- (20260826063500_scheduling_api.sql) exactly: a real-instant half-open
-- range, computed client-side from the viewed calendar day
-- (`lib/localTime.ts` `localDayRange`), not a `date` column comparison --
-- `note_entries.created_at` is a `timestamptz`, and the day boundary is the
-- device's own local midnight, so the range has to be resolved to real
-- instants by the caller (same reasoning `list_scheduled_activities` already
-- follows for its own `local_date`-adjacent rows).
--
