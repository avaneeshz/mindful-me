...
- ✅ **Protective response** (formerly "Flag") — Trauma Activation / Triggered / Attack / Anger / Procrastinated / Shut Down / Collapse / Over Accommodating / Hyper Responsibility / Over Function / Intellectualization / Optimization / Hyper Vigilance / Problem Solving, plus None (ordered last) — a single-select, optional pick on an individual scheduled activity (at most one), encrypted at rest, text-only chips (SCRUM-15).
- ✅ **Activity quality** (formerly "How did it feel?") — a multi-select, optional reflection on an individual scheduled activity (18 values, SCRUM-10), encrypted at rest, text-only chips (SCRUM-15).
- ✅ **Chronic Symptoms** — a multi-select, optional set of symptoms noticed around a logged activity (Pitta, Inflammation, Right knee pain, Calves pain, Temporal pain, Dryness — any number at once), encrypted at rest, text-only chips (SCRUM-15).
