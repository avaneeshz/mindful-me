 ...
 * together in one view, before Save commits everything through the exact
 * same `commit` reducer action the side panel always used.
 *
    ...
    <Dialog.Root open={isOpen} onOpenChange={(open) => !open && onCancel()}>
      ...
      >
        ...
        {card && (
          <div className="flex items-start justify-between gap-md">
            <div className="min-w-0">
              ...
              <p className="mt-xs text-caption font-medium text-muted">{categoryOf(card.name).label}</p>
            </div>
        ...
        {/* Sub/third-level drill-down chips — the SAME staging/path
            ...
            the modal. Shown TOGETHER with duration/quality/flag below, not
            as a sequential either/or step. */}
        {options && (
