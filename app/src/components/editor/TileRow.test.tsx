...
describe('tile progress — spoken via aria-label, no visible "x/y" badge any more', () => {
  ...
  it('counts an auto-locked item once its scheduled-today count reaches its limit', () => {
    const activities: ActivityList = [
      { id: 'a1', name: 'Night Sleep', path: [], startMinutes: 0, durationMinutes: 480, flags: [], quality: [], symptoms: [], notes: null, reflections: [], fieldSelections: {}, dreamsNote: null, status: 'planned', timezone: 'UTC' },
    ]
...
describe('whole-tile lock treatment', () => {
  it('marks the whole tile locked once every one of its own items is locked', () => {
    const activities: ActivityList = [
      { id: 'a1', name: 'Night Sleep', path: [], startMinutes: 0, durationMinutes: 30, flags: [], quality: [], symptoms: [], notes: null, reflections: [], fieldSelections: {}, dreamsNote: null, status: 'planned', timezone: 'UTC' },
      { id: 'a2', name: 'Day Sleep', path: [], startMinutes: 60, durationMinutes: 30, flags: [], quality: [], symptoms: [], notes: null, reflections: [], fieldSelections: {}, dreamsNote: null, status: 'planned', timezone: 'UTC' },
      { id: 'a3', name: 'Bed Exercise', path: [], startMinutes: 120, durationMinutes: 15, flags: [], quality: [], symptoms: [], notes: null, reflections: [], fieldSelections: {}, dreamsNote: null, status: 'planned', timezone: 'UTC' },
      { id: 'a4', name: 'Bed Exercise', path: [], startMinutes: 150, durationMinutes: 15, flags: [], quality: [], symptoms: [], notes: null, reflections: [], fieldSelections: {}, dreamsNote: null, status: 'planned', timezone: 'UTC' },
    ]
    const dismissed = new Set(['Supplements', 'Slow down', 'Sleep'])
    const sleepCount = cardsForCategory('sleep').length
...
describe('monochrome, flat-progress-bar tile row (Section A/B — no colour anywhere)', () => {
  ...
  it('renders no separate numeric "x/y" badge on the tile — done/total is now spoken only, via aria-label', () => {
    ...
    const html = renderRow({ activities })
    const sleepCount = cardsForCategory('sleep').length
    // "1/N" (the old visible badge format) does not appear anywhere in the
    // tile markup; the aria-label still carries the real count in words.
    expect(html).not.toContain(`>1/${sleepCount}<`)
    expect(html).toContain(`Sleep &amp; Rest, 1 of ${sleepCount} done`)
  })
  ...
  it('the progress bar fill is a real done/total WIDTH percentage (Section B — replaces the old water-fill HEIGHT gauge)', () => {
    ...
    const sleepCount = cardsForCategory('sleep').length
    const expectedPct = Math.round((1 / sleepCount) * 100)
    const html = renderRow({ activities }) // Sleep & Rest: 1 of N
    expect(html).toContain(`width:${expectedPct}%`)
