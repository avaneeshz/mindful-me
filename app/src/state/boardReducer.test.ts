...
describe('toggleStagingFieldSelection / setStagingDreamsNote — configured note fields (e.g. Sleep)', () => {
  ...
  it('setStagingDreamsNote stages exactly the typed text, replacing any prior draft — a SEPARATE field from notes', () => {
    let state = boardReducer(start(), { type: 'pickCard', cardName: 'Sleep' })
    state = boardReducer(state, { type: 'setStagingNotes', notes: 'Slept in the guest room.' })
    state = boardReducer(state, { type: 'setStagingDreamsNote', note: 'Flying again.' })
    expect(state.staging.notes).toBe('Slept in the guest room.')
    expect(state.staging.dreamsNote).toBe('Flying again.')
  })

  it('commit attaches both staged fields to the real activity; an empty/blank dreams draft commits as null, never an empty string', () => {
    let state = run(
      start(),
...
describe('quickLogActivity — Exercise/Sleep (path/type), and notes/fieldSelections/dreamsNote passthrough', () => {
  ...
  it('carries the chosen type through as `path`, same shape a tile-row sub-pick produces', () => {
    const state = boardReducer(start(), {
      type: 'quickLogActivity',
      cardName: 'Sports or Exercise',
      startMinutes: 6 * 60,
      durationMinutes: 30,
      path: ['HIIT'],
    })
    expect(real(state)[0].path).toEqual(['HIIT'])
  })

  it('carries a plain note through to the real activity notes field', () => {
    const state = boardReducer(start(), {
      type: 'quickLogActivity',
      cardName: 'Breathwork',
      startMinutes: 6 * 60,
    ...
    expect(real(state)[0].notes).toBe('Box breathing.')
  })

  it('a blank/whitespace-only note commits as null, never an empty string', () => {
    const state = boardReducer(start(), {
      type: 'quickLogActivity',
      cardName: 'Breathwork',
      startMinutes: 6 * 60,
  ...
  it('multiple Sleep sessions in one day (Night sleep + a Nap) are two separate rows, both counted in the Sleep total', () => {
    let state = boardReducer(start(), {
      type: 'quickLogActivity',
      cardName: 'Sleep',
      startMinutes: 0,
    ...
    const sessions = real(state).filter((a) => a.name === 'Sleep')
    expect(sessions).toHaveLength(2)
    expect(sessions.reduce((sum, a) => sum + a.durationMinutes, 0)).toBe(6 * 60 + 45)
