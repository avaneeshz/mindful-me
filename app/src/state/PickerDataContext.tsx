 ...
 * `BoardProvider`) and the (then separate-route) Activity Library page each
 * called `useTiles()`/`useActivityHierarchy()` independently, minting two
 * entirely separate sets of React state backed by the same server rows.
 * Editing a tile/activity there updated only ITS OWN hook instance — the
 * live picker's separate instance (and therefore `data/activities.ts`'s live
 * catalog registry) never saw the change until a full reload remounted
 * `BoardProvider` from scratch.
 *
