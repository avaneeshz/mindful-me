...
Tailwind CSS + shadcn/ui-style components + Radix UI primitives + Lucide icons + Recharts for charts + Motion for the small amount of purposeful animation the product actually uses. No other UI framework. Emoji are never used as primary interface icons.
...
**Chart color is reinforcement, not identity.** The existing category palette was tuned for filled tiles, not thin chart marks — rather than introduce a second, chart-specific palette (which would fight "one coherent design system"), Insights charts keep the product's category colors as secondary reinforcement, with the axis label or legend chip carrying the actual identity signal. A deliberate, documented trade-off, not an oversight.
...
A two-value ratio against a limit (free vs. occupied time, a completion percentage) is rendered as a **meter** — extending the scheduling editor's existing `CapacityMeter` visual language — never as a two-slice pie chart. A real trend or a multi-category breakdown, where a shape genuinely helps, uses a Recharts bar chart instead.
...
The header's sync indicator is silent when everything is healthy — no permanent "Synced ✓" badge — and only appears to say something worth knowing (offline, syncing, can't sync, updated elsewhere). Restraint applies to status chrome exactly as much as it applies to decoration.
